...
const TYPE_COLORS = {
  ...
  instagram:  { bg: "#2E0A1A", color: "#FF6EB0" },
  tweet:      { bg: "#0A1420", color: "#4DB8FF" },
  link:       { bg: "#0A1A0A", color: "#CCFF00" },
  tekst:      { bg: "#1A1A0A", color: "#FFD166" },
  afbeelding: { bg: "#0A1A18", color: "#44FFCC" },
  url:        { bg: "#0A1A0A", color: "#CCFF00" },
};
      ...
      <div style={s.root}>
        ...
        <div style={s.topbar}>
          ...
          <div style={s.topActions}>
            <button onClick={() => setViewMode(viewMode === "list" ? "grid" : "list")} style={s.iconBtn} title="Wissel weergave">
              {viewMode === "list" ? "⊞" : "☰"}
            </button>
        ...
        {loading ? (
          ...
          <div style={s.grid}>
            {items.map((item) => <GridCard key={item.id} item={item} img={img(item)} deleteId={deleteId} setDeleteId={setDeleteId} handleDelete={handleDelete} formatDate={formatDate} decodeEntities={decodeEntities} activeTag={activeTag} setActiveTag={setActiveTag} tc={TYPE_COLORS[item.type] || TYPE_COLORS.link} />)}
          </div>
        ) : (
          <div style={s.list}>
            {items.map((item) => <ListCard key={item.id} item={item} img={img(item)} deleteId={deleteId} setDeleteId={setDeleteId} handleDelete={handleDelete} formatDate={formatDate} decodeEntities={decodeEntities} expandedId={expandedId} setExpandedId={setExpandedId} activeTag={activeTag} setActiveTag={setActiveTag} tc={TYPE_COLORS[item.type] || TYPE_COLORS.link} />)}
          </div>
        )}
      </div>
    </>
...
function GridCard({ item, img, deleteId, setDeleteId, handleDelete, formatDate, decodeEntities, activeTag, setActiveTag, tc }) {
  const name = decodeEntities(item.name);
  const desc = decodeEntities(item.og_description || item.content || "");
  return (
    <div style={s.gridCard}>
      {img && (
        <div style={s.gridImgWrap}>
          <img src={img} alt="" style={s.gridImg} onError={(e) => { e.target.parentElement.style.display = "none"; }} />
        </div>
      )}
      <div style={s.gridBody}>
        ...
        <div style={s.gridName}>
          {item.url ? <a href={item.url} target="_blank" rel="noreferrer">{name}</a> : name}
        </div>
        {desc && <div style={s.gridDesc}>{desc.slice(0, 80)}{desc.length > 80 ? "..." : ""}</div>}
        {item.tags?.length > 0 && (
          <div style={s.cardTags}>
            {item.tags.map((t) => <span key={t} onClick={() => setActiveTag(t)} style={{ ...s.tagMini, ...(activeTag === t ? s.tagMiniActive : {}) }}>{t}</span>)}
          </div>
        )}
        {deleteId === item.id && (
          <div style={s.delConfirm}>
            <span style={s.delTxt}>Verwijderen?</span>
            <button onClick={() => handleDelete(item.id)} style={s.delYes}>Ja</button>
            <button onClick={() => setDeleteId(null)} style={s.delNo}>Nee</button>
          </div>
        )}
      </div>
    </div>
...
function ListCard({ item, img, deleteId, setDeleteId, handleDelete, formatDate, decodeEntities, expandedId, setExpandedId, activeTag, setActiveTag, tc }) {
    ...
    <div style={s.listCard}>
      ...
      <div style={s.listBody}>
        ...
        <div style={s.listName}>
          {item.url ? <a href={item.url} target="_blank" rel="noreferrer" style={{ color: "inherit" }}>{name}</a> : name}
        </div>
        {desc && <div style={s.listDesc}>{desc.slice(0, 130)}{desc.length > 130 ? "..." : ""}</div>}
        {item.url && <div style={s.urlTxt}>{item.url.replace(/^https?:\/\//, "").slice(0, 55)}{item.url.length > 60 ? "..." : ""}</div>}
        {item.tags?.length > 0 && (
          <div style={s.cardTags}>
            {item.tags.map((t) => <span key={t} onClick={() => setActiveTag(t)} style={{ ...s.tagMini, ...(activeTag === t ? s.tagMiniActive : {}) }}>{t}</span>)}
          </div>
...
const s = {
  root: { minHeight: "100vh", background: "var(--bg)", padding: "20px 16px 60px", maxWidth: 680, margin: "0 auto" },

  // Topbar
  topbar: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 },
  logoWrap: { display: "flex", alignItems: "center", gap: 10 },
  logo: { fontWeight: 700, fontSize: 20, color: "#fff", letterSpacing: "-0.02em" },
  ...
  addBtn: { background: "var(--accent)", color: "var(--accent-text)", fontWeight: 700, fontSize: 13, padding: "8px 16px", borderRadius: 10, display: "inline-block", letterSpacing: "-0.01em" },
  ...
  gridImgWrap: { height: 130, overflow: "hidden" },
  ...
  imgHint: { textAlign: "center", padding: "4px 0 6px", fontSize: 10, color: "var(--muted)", letterSpacing: "0.05em" },
