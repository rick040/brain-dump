...
// Extract URL from a string (Instagram/apps often put URL in text field)
...
export default async function handler(req, res) {
  ...
  let text = field(fields.text);
  let url = field(fields.url);
  const mediaFile = Array.isArray(files.media) ? files.media[0] : files.media;
 
  // Many apps (Instagram, Chrome Android) put the URL in text, not url
  if (!url) {
    const urlInText = extractUrl(text);
    if (urlInText) {
      url = urlInText;
      text = text.replace(urlInText, "").trim();
    }
  }
 
  // If title is actually a URL, move it
